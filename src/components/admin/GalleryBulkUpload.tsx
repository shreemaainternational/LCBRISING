 ...
 *
 * Picking an event links the whole batch to it (`photos.activity_id`), which
 * files the photos into that event's album on both surfaces; leaving it
 * unset drops them into the general gallery.
 */
export function GalleryBulkUpload() {
  ...
  // Events (activities) to file this batch's album under. Loaded once —
  // picking one links every photo in the batch via `photos.activity_id`.
  useEffect(() => {
    fetch('/api/activities?limit=300')
      .then((res) => res.json())
      .then((j) => {
        if (Array.isArray(j?.activities)) setEvents(j.activities);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-4">
        ...
        <label className="text-sm">
          <span className="block mb-1 text-gray-600">Event / album (optional)</span>
          <select
            value={activityId}
            onChange={(e) => setActivityId(e.target.value)}
            className="rounded-md border border-gray-300 px-3 py-2 bg-white text-sm max-w-[280px]"
          >
