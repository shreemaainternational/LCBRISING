...
export default async function ZoneNotificationsPage() {
  const ctx = await requireZoneChair();
  const db = createAdminClient();
  const { data: advisories } = await db.from('advisories')
    .select('id, subject, body, priority, status, created_at, clubs(name)')
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 text-sm mt-1">Activity in {ctx.zone.name}.</p>
      </div>
      <ZoneTabs />
