...
function friendlyError(message: string): string {
  ...
  if (/duplicate key/i.test(message)) {
    return 'A region with that code already exists in this district.';
  }
  return message;
}
...
export async function GET() {
  ...
  const { data, error } = await supa.from('regions').select('*').is('deleted_at', null).order('name');
...
export async function POST(req: Request) {
  ...
  const payload: Record<string, unknown> = {
    ...
    chairperson_name: parsed.data.chairperson_name ?? parsed.data.region_chairperson_name,
  };
  for (const k of Object.keys(payload)) if (payload[k] == null || payload[k] === '') delete payload[k];

  // regions.code is NOT NULL — auto-derive when omitted.
  if (!payload.code) {
    const base = String(parsed.data.name).toUpperCase().replace(/[^A-Z0-9]+/g, '').slice(0, 12) || 'REGION';
    payload.code = `${base}-${Date.now().toString(36).slice(-4).toUpperCase()}`;
  }

  // A region must belong to a district — resolve-or-bootstrap the default.
