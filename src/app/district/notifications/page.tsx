...
export default async function DistrictNotificationsPage() {
  ...
  const { data: advisories } = await db.from('advisories')
    .select('id, subject, body, priority, status, created_at, clubs(name), zones(name)')
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 text-sm mt-1">All advisories filed across District {ctx.district.code}.</p>
      </div>
      <DistrictTabs />
              ...
              <li key={a.id} className="p-4 flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[11px] text-gray-500 mt-1">
                    {(a.zones as { name?: string } | null)?.name ?? 'District'}
                    {' · '}{(a.clubs as { name?: string } | null)?.name ?? '—'}
