...
-- ============== 4. REAL ACTIVITY — TB Kit Distribution (26 Dec 2024) =====
-- Club activity report (Activity No. 2). Surfaces on the public home page
-- "Recent Activities" and in the CRM admin activities list (both read
-- public.activities). Idempotent; guarded on title + date.
...
begin
  if to_regclass('public.clubs') is not null then
    execute $q$ select id from public.clubs
                 where name = 'Lions Club of Baroda Rising Star' limit 1 $q$
      into v_club_id;
  end if;

  insert into public.activities (
    club_id, title, description, category, beneficiaries, service_hours,
    volunteer_hours_total, lion_members_count, amount_raised, budget,
    expenses, date, location, is_medical_camp, status, reported_to_district
  )
  select
    v_club_id, 'TB Kit Distribution',
    ...
    'healthcare', 620, 32, 32, 16, 0, 5500, 5500, date '2024-12-26',
    'Urban Health Center, Warasiya, Vadodara', true, 'completed', false
  where not exists (
    select 1 from public.activities
     where title = 'TB Kit Distribution' and date = date '2024-12-26'
  );
  raise notice 'TB Kit Distribution activity ensured.';
end $$;
